...
class ShippingcourierController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $user = User::find($request->courier);
        ...
        foreach ($request['order_ids'] as $order_id) {
            ...
            $order->status_awb = "Kurir sudah ditugaskan untuk mengirimkan pesanan";
            $order->save();

            HistoryAwb::create([
                'order_id' => $order->id,
                'awb'      => $order->numberorders,
                'status'   => "Kurir " . $user->name . " sudah ditugaskan untuk mengirimkan pesanan",
