                            ...
                            <div class="col-md-6 col-lg-6 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select name="destination" id="destination" class="form-control"
                                        onchange="manifestOrder(this.value)" disabled>
                                        <option value="">-- Pilih Destination --</option>
                                        @foreach ($destination as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $suratjalan->destinations_id == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
...
@section('custom-js')
    ...
    <script>
        ...
        const getList = () => {
            $.getJSON(urlBase + '/' + listRoutes['traveldocument.listDetailEdit'].replace('{id}', suratJalanId),
        ...
        function hapusManifest(x, i) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: window.location.origin + '/' + listRoutes['traveldocument.deleteDetail']
                            .replace(
                                '{id}', i),
                        type: "POST",
                        dataType: "JSON",
        ...
        $('#formEditSuratJalan').validate({
            ...
            submitHandler: function() {
                $.ajax({
                    url: window.location.origin + '/' + listRoutes['traveldocument.update'].replace(
                        '{id}', suratJalanId),
                    type: "POST",
                    dataType: "JSON",
